...
-- In
-- a) Todos los nombres de países con una ciudad de más de 9269265 habitantes
SELECT DISTINCT c.Name 
FROM country c
WHERE c.Code IN (
    SELECT ci.CountryCode 
    FROM city ci 
    WHERE ci.Population > 9269265
);

-- b) Todos los nombres de países donde se hable español (‘Spanish’)
SELECT DISTINCT c.Name 
FROM country c
WHERE c.Code IN (
...
-- c) Todos los nombres de los países donde el porcentaje de su idioma oficial este entre el 50% y el 80%
SELECT DISTINCT c.Name 
FROM country c
WHERE c.Code IN (
    SELECT cl.CountryCode 
    ...
    AND cl.Percentage BETWEEN 50 AND 80
);

-- d) Todos los nombres de países que tengan una ciudad que el tamaño de su nombre no sobrepase los 3 caracteres
SELECT DISTINCT c.Name 
FROM country c
WHERE c.Code IN (
    SELECT ci.CountryCode 
    FROM city ci 
    WHERE LENGTH(ci.Name) <= 3
);

-- e) Todos los nombres de ciudades que pertenecen a Europa (‘Europe’)
SELECT ci.Name 
FROM city ci
...
-- f) Todos los nombres de ciudades que son capital de algún país
SELECT ci.Name 
FROM city ci
WHERE ci.ID IN (
    SELECT c.Capital 
...
-- Having
-- g) El nombre del país y el número de ciudades de los países con más de 200 ciudades
SELECT c.Name, COUNT(ci.ID) AS NumCiudades
FROM country c
JOIN city ci ON c.Code = ci.CountryCode
GROUP BY c.Code, c.Name
HAVING COUNT(ci.ID) > 200;

-- h) El nombre del país y la población promedio de sus ciudades de los países con entre 50 y 100 ciudades
SELECT c.Name, AVG(ci.Population) AS PromedioPoblacion
FROM country c
JOIN city ci ON c.Code = ci.CountryCode
GROUP BY c.Code, c.Name
HAVING COUNT(ci.ID) BETWEEN 50 AND 100;

-- i) El nombre del país y la densidad de población de los países con más de 1500 habitantes por unidad de superficie
SELECT c.Name, (c.Population / c.SurfaceArea) AS DensidadPoblacion
FROM country c
WHERE (c.Population / c.SurfaceArea) > 1500;

-- Procedimientos almacenados
-- Procedimiento 1: Registrar un nuevo país
DELIMITER //

DROP PROCEDURE IF EXISTS RegistrarNuevoPais //
CREATE PROCEDURE RegistrarNuevoPais(
    IN p_code CHAR(3),
    IN p_name CHAR(52),
    IN p_continent VARCHAR(20),
    IN p_region CHAR(26),
    IN p_surface_area DECIMAL(10,2),
    IN p_population INT,
    IN p_local_name CHAR(45),
    IN p_government_form CHAR(45),
...
BEGIN
    ...
    SELECT COUNT(*) INTO existe FROM country WHERE Code = p_code;
    
    IF existe > 0 THEN
        SET p_mensaje = 'Error: El código de país ya existe';
        SET p_exito = FALSE;
    ELSE
        -- Insertar nuevo país
        INSERT INTO country (
            Code, Name, Continent, Region, SurfaceArea, 
            Population, LocalName, GovernmentForm, IndepYear,
            LifeExpectancy, GNP, GNPOld, HeadOfState, Capital, Code2
        ) VALUES (
            p_code, p_name, p_continent, p_region, p_surface_area,
            p_population, p_local_name, p_government_form, NULL,
            NULL, NULL, NULL, NULL, NULL, SUBSTRING(p_code, 1, 2)
        );
        
        SET p_mensaje = CONCAT('País ', p_name, ' registrado exitosamente');
        SET p_exito = TRUE;
    END IF;
END //
...
-- Procedimiento 2: Calcular estadísticas de un país (población total, número de ciudades, idiomas)
DELIMITER //

DROP PROCEDURE IF EXISTS EstadisticasPais //
CREATE PROCEDURE EstadisticasPais(
    IN p_country_code CHAR(3),
    OUT p_nombre_pais VARCHAR(100),
    OUT p_poblacion_total INT,
    OUT p_numero_ciudades INT,
    OUT p_numero_idiomas INT,
    OUT p_superficie DECIMAL(10,2),
    OUT p_densidad DECIMAL(10,2)
)
BEGIN
    -- Obtener información del país
    SELECT Name, Population, SurfaceArea 
    INTO p_nombre_pais, p_poblacion_total, p_superficie
    FROM country 
    WHERE Code = p_country_code;
    
    -- Contar ciudades
    SELECT COUNT(*) INTO p_numero_ciudades
    FROM city 
    WHERE CountryCode = p_country_code;
    
    -- Contar idiomas
    SELECT COUNT(*) INTO p_numero_idiomas
    FROM countrylanguage 
    WHERE CountryCode = p_country_code;
    
    -- Calcular densidad
    IF p_superficie > 0 THEN
        SET p_densidad = p_poblacion_total / p_superficie;
    ELSE
        SET p_densidad = 0;
    END IF;
END //
...
-- Procedimiento 3: Actualizar población de un país basado en suma de poblaciones de sus ciudades
DELIMITER //

DROP PROCEDURE IF EXISTS ActualizarPoblacionPorCiudades //
CREATE PROCEDURE ActualizarPoblacionPorCiudades(
    IN p_country_code CHAR(3),
    OUT p_nueva_poblacion INT,
    OUT p_mensaje VARCHAR(100)
)
BEGIN
    DECLARE suma_ciudades INT DEFAULT 0;
    DECLARE existe INT DEFAULT 0;
    
    -- Verificar si el país existe
    SELECT COUNT(*) INTO existe FROM country WHERE Code = p_country_code;
    
    IF existe = 0 THEN
        SET p_mensaje = 'Error: País no encontrado';
        SET p_nueva_poblacion = 0;
    ELSE
        -- Calcular suma de poblaciones de ciudades
        SELECT SUM(Population) INTO suma_ciudades
        FROM city 
        WHERE CountryCode = p_country_code;
        
        -- Actualizar población del país
        UPDATE country 
        SET Population = suma_ciudades 
        WHERE Code = p_country_code;
        
        SET p_nueva_poblacion = suma_ciudades;
        SET p_mensaje = CONCAT('Población actualizada a ', suma_ciudades);
    END IF;
END //
...
CALL RegistrarNuevoPais('XYZ', 'Nuevo País', 'Asia', 'Central Asia', 50000, 1000000, 'Nuevo Pais', 'Republic', @mensaje, @exito);
SELECT @mensaje, @exito;
DELETE FROM country WHERE Code = 'XYZ';

CALL EstadisticasPais('COL', @nombre, @poblacion, @ciudades, @idiomas, @superficie, @densidad);
SELECT @nombre AS Pais, @poblacion AS Poblacion, @ciudades AS Ciudades, @idiomas AS Idiomas, @densidad AS Densidad;

CALL ActualizarPoblacionPorCiudades('COL', @nueva_pob, @mensaje);
SELECT @nueva_pob, @mensaje;

-- Funciones
-- Función 1: Calcular densidad poblacional de un país
DELIMITER //

DROP FUNCTION IF EXISTS CalcularDensidadPoblacional //
CREATE FUNCTION CalcularDensidadPoblacional(
    p_country_code CHAR(3)
)
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
    DECLARE poblacion INT DEFAULT 0;
    DECLARE superficie DECIMAL(10,2) DEFAULT 0;
    DECLARE densidad DECIMAL(10,2) DEFAULT 0;
    
    -- Obtener datos del país
    SELECT Population, SurfaceArea 
    INTO poblacion, superficie
    FROM country 
    WHERE Code = p_country_code;
    
    -- Calcular densidad
    IF superficie > 0 THEN
        SET densidad = poblacion / superficie;
    END IF;
...
-- Función 2: Contar el número de idiomas oficiales de un país
DELIMITER //

DROP FUNCTION IF EXISTS ContarIdiomasOficiales //
CREATE FUNCTION ContarIdiomasOficiales(
    p_country_code CHAR(3)
)
RETURNS INT
DETERMINISTIC
BEGIN
    DECLARE total_idiomas INT DEFAULT 0;
    
    SELECT COUNT(*) INTO total_idiomas
    FROM countrylanguage
    WHERE CountryCode = p_country_code 
    AND IsOfficial = 'T';
...
-- Función 3: Calcular el porcentaje de población mundial de un país
DELIMITER //

DROP FUNCTION IF EXISTS PorcentajePoblacionMundial //
CREATE FUNCTION PorcentajePoblacionMundial(
    p_country_code CHAR(3)
)
RETURNS DECIMAL(10,4)
DETERMINISTIC
BEGIN
    DECLARE poblacion_pais BIGINT DEFAULT 0;
    DECLARE poblacion_total_mundial BIGINT DEFAULT 0;
    DECLARE porcentaje DECIMAL(10,4) DEFAULT 0;
    
    -- Obtener población del país
    SELECT Population INTO poblacion_pais
    FROM country 
    WHERE Code = p_country_code;
    
    -- Obtener población mundial total
    SELECT SUM(Population) INTO poblacion_total_mundial
    FROM country;
    
    -- Calcular porcentaje
    IF poblacion_total_mundial > 0 THEN
        SET porcentaje = (poblacion_pais * 100.0) / poblacion_total_mundial;
    END IF;
...
SELECT 
    Name,
    CalcularDensidadPoblacional(Code) AS Densidad_hab_km2,
    ContarIdiomasOficiales(Code) AS Idiomas_Oficiales,
    PorcentajePoblacionMundial(Code) AS Porcentaje_Mundial
FROM country 
WHERE Code IN ('COL', 'USA', 'CHN', 'BRA');

-- Triggers
-- Tabla de auditoría para países (creamos solo esta tabla auxiliar necesaria para triggers)
...
-- Trigger 1: Auditar actualizaciones de países
DELIMITER //

DROP TRIGGER IF EXISTS AuditarActualizacionPais //
CREATE TRIGGER AuditarActualizacionPais
...
BEGIN
    ...
    IF OLD.Population != NEW.Population THEN
        SET cambios = CONCAT(cambios, 'Población: ', OLD.Population, ' → ', NEW.Population, '; ');
    END IF;
    
    IF OLD.SurfaceArea != NEW.SurfaceArea THEN
        SET cambios = CONCAT(cambios, 'Superficie: ', OLD.SurfaceArea, ' → ', NEW.SurfaceArea, '; ');
    END IF;
    
    IF OLD.Continent != NEW.Continent THEN
        SET cambios = CONCAT(cambios, 'Continente: ', OLD.Continent, ' → ', NEW.Continent, '; ');
    END IF;
    
    IF cambios != '' THEN
        INSERT INTO auditoria_paises (country_code, accion, datos_anteriores, datos_nuevos, usuario, fecha_cambio)
        VALUES (NEW.Code, 'UPDATE', OLD.Name, cambios, USER(), NOW());
    END IF;
END //

DELIMITER ;

-- Trigger 2: Auditar inserción de nuevos países
DELIMITER //

DROP TRIGGER IF EXISTS AuditarInsercionPais //
CREATE TRIGGER AuditarInsercionPais
AFTER INSERT ON country
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_paises (country_code, accion, datos_anteriores, datos_nuevos, usuario, fecha_cambio)
    VALUES (NEW.Code, 'INSERT', NULL, 
            CONCAT('País: ', NEW.Name, ', Población: ', NEW.Population, ', Continente: ', NEW.Continent),
            USER(), NOW());
END //

DELIMITER ;

-- Trigger 3: Auditar eliminación de países
DELIMITER //

DROP TRIGGER IF EXISTS AuditarEliminacionPais //
CREATE TRIGGER AuditarEliminacionPais
AFTER DELETE ON country
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_paises (country_code, accion, datos_anteriores, datos_nuevos, usuario, fecha_cambio)
    VALUES (OLD.Code, 'DELETE', 
            CONCAT('País: ', OLD.Name, ', Población: ', OLD.Population),
            NULL, USER(), NOW());
END //

DELIMITER ;

-- Trigger 4: Validar que la población no sea negativa antes de insertar
DELIMITER //

DROP TRIGGER IF EXISTS ValidarPoblacionPais //
CREATE TRIGGER ValidarPoblacionPais
BEFORE INSERT ON country
FOR EACH ROW
BEGIN
    IF NEW.Population < 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error: La población no puede ser negativa';
    END IF;
    
    IF NEW.SurfaceArea < 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error: La superficie no puede ser negativa';
    END IF;
END //

DELIMITER ;

-- Trigger 5: Actualizar automáticamente el código2 cuando se inserta un país
DELIMITER //

DROP TRIGGER IF EXISTS ActualizarCode2 //
CREATE TRIGGER ActualizarCode2
BEFORE INSERT ON country
FOR EACH ROW
BEGIN
    IF NEW.Code2 IS NULL OR NEW.Code2 = '' THEN
        SET NEW.Code2 = SUBSTRING(NEW.Code, 1, 2);
    END IF;
END //

DELIMITER ;

-- Ejemplos de prueba de triggers
-- Insertar un nuevo país (activa trigger AuditarInsercionPais)
INSERT INTO country (Code, Name, Continent, Region, SurfaceArea, Population, LocalName, GovernmentForm, Code2)
VALUES ('TST', 'Test Country', 'Asia', 'Test Region', 1000, 5000000, 'Test', 'Republic', 'TS');
